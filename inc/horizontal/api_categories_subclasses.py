...
# If this enabled, it will add a generic description to table row, otherwise, it won't.
...
async def save_random_props(root_path, df, filename, common_description=None, are_subclasses=True):
    ...
    target_path = join(config.HORIZONTAL_PATH, 'subclasses', root_path, folder_suffix)
...
async def handle_results(subclasses, res_strs, are_subclasses=True):
    ...
    for k, v in subclasses.items():
        ...
        if instances:
            ...
            prop_dict = {}
            res_props = {}
            for batch in batched(instances, 200):
                try:
            ...
            if len(index_dict) > 0:
                # save a table with all these entities without any intersection /all  "common" props those with
                # support > 2
            ...
            for shared_entities, their_props in res:
                ...
                await save_table(root_path='common_props', filename='{}_{}.csv'.format(k, i),
                                 cols=their_props, rows=rows_obj, common_description=cate_desc,
