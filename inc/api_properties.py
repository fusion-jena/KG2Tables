...
wiki_props = inc.cache.Cache('wiki_props', ['base'])
...
async def get_wiki_props_for_lst(entities, lang='en'):
    """
    get all string properties for the given entities (labels, alias, description)
    primary properties are those with the highest rank per property
    see https://www.wikidata.org/wiki/Help:Ranking
    ...
    res = await runQuerySingleKey(wiki_props, entities, """
     SELECT ?base ?p ?pLabel ?propLabel ?o ?oLabel
        WHERE 
        {
          VALUES ?base { %s }
          ?base ?p ?o.    
          ...
          # excludes firebase ID, Google Graph ID ... etc.
          FILTER(!CONTAINS(?propLabel, "ID")).
          
          # exclude images.
          FILTER(?p!=wdt:P18 ).          
          
          # exclude supercass and instance of prop.
          FILTER(?p!=wdt:P279 ).
          FILTER(?p!=wdt:P31 ).
